...
export default function WriteForm({ boardId, slug }) {
  ...
  async function handleSubmit(e){
    ...
    for (const f of files) {
      ...
      const { data, error } = await sb.storage.from('images-public').upload(path, f, { upsert:false })
      if (error) { console.error(error); continue }
      const publicUrl = sb.storage.from('images-public').getPublicUrl(data.path).data.publicUrl
      urls.push(publicUrl)
    }

    // 2) 글 저장 (포인트는 DB 트리거가 처리)
    ...
    r.push(`/boards/${slug}/${data.id}`)
    r.refresh()
  }

  return (
    <form onSubmit={handleSubmit} className="card p-3 md:p-4 grid gap-3">
      ...
      <div className="flex justify-end">
        <button disabled={loading} className="btn btn-primary">{loading?'등록 중…':'등록'}</button>
      </div>
    </form>
  )
}
